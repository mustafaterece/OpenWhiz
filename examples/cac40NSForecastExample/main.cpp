...
int main() {
    try {
...
#else
        std::cout << "Using CPU (SIMD Accelerated)" << std::endl;
#endif
        ...
        std::cout << "Total Training Time: " << std::fixed << std::setprecision(2) << durationMs << " ms" << std::endl;
        std::cout << "Avg Time per Epoch: " << (durationMs / nn.getTrainingEpochNum()) << " ms" << std::endl;
